...
void blackTail()
{
    strcpy(pSnakeHead->node.text, BG);  // 用背景覆盖掉尾部？是吗
...
void updateKey(void)
{
    ...
    switch (sys_gs)
    {
        ...
        case GAMING:
            ...
            {
                sys_gs=PAUSE;
                printf("Pause! [enter] to continue! [Esc] to exit!\t");
...
void updateSnake(void)
{
    ...
    if (!checkSnake())
    {
        sys_gs=DIE;
        gameover();
        printf("Game Over! [1-9] or [Enter] to continue! [Esc] to exit!\t");
        JR_SetCursor(0,ROW+1);
    }
}
